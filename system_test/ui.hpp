	...
	void render(Rectangle rect) {
		...
		switch(style.background.type) {
			case BackgroundType::Color: {
				DrawRectangleRec(_rect, style.background.data.color);
			} break;
			case BackgroundType::Image: {
				DrawTexture(style.background.data.texture, _rect.x, _rect.y, WHITE);
			} break;
			case BackgroundType::TextureAtlas: {
				style.background.data.drawer((Vector2){_rect.x, _rect.y}, 
					(Vector2){_rect.width, _rect.height}, style.disabled ? GRAY: WHITE);
			} break;
		}
